...
namespace HelpDeskWinFormsApp
{
    public partial class ExportForm : Form
    {
        ...
        public ExportForm(bool isSupport)
        {
            InitializeComponent();
        ...
        private void ExportButton_Click(object sender, EventArgs e)
        {
            ...
            var trubleTickets = DbProvider.GetAllTrubleTickets().Where(t => t.Created >= startDateTimePicker.Value && t.Created <= endDateTimePicker.Value).ToList();
            var users = DbProvider.GetAllUsers();
            ...
            if (typeComboBox.Text == "Truble Ticket")
            {
                ...
                {
                    trubleTickets = trubleTickets.Where(t => t.Status == statusFilterComboBox.Text).ToList();
                }
            ...
            else if (typeComboBox.Text == "Пользователи")
            {
                ...
                if (statusFilterCheckBox.Checked)
                {
                    ...
                    {
                        users = users.Where(u => u is not Employee).ToList();
                        onlyClientsExport = true;
                    }
                    else if (statusFilterComboBox.Text == "Сотрудник")
                    {
                        users = users.Where(u => u is Employee).ToList();
                    }
                }
        ...
        private void CsvUsersExport(List<User> users, string exportFile, bool onlyClients)
        {
            ...
            using (StreamWriter sw = new StreamWriter(exportFile, false, Encoding.UTF8))
            {
                ...
                {
                    var isEmployee = users[i] is Employee;
                    ...
                    if (!onlyClients)
                    {
                        ...
                        {
                            sw.Write(";");
                            sw.Write("\"" + ((Employee)users[i]).Function + "\";");
                            sw.Write("\"" + ((Employee)users[i]).Department + "\"\r\n");
                        }
                        else
                        {
                            sw.Write(";\"\";\"\"\r\n");
        ...
        private void ExcelUsersExport(List<User> users, string exportFile, bool onlyClients)
        {
            ...
            for (int i = 0; i < userCount; i++)
            {
                ...
                sheet.Cell(i + 2, 6).SetValue(isEmployee ? ((Employee)users[i]).Function : string.Empty);
                sheet.Cell(i + 2, 7).SetValue(isEmployee ? ((Employee)users[i]).Department : string.Empty);
            }
