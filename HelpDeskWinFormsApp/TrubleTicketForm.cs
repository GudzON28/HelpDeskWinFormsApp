...
namespace HelpDeskWinFormsApp
{
    public partial class TrubleTicketForm : Form
    {
        ...
        public TrubleTicketForm(int ticketId, bool isEmployee, int resolveUserId)
        {
            InitializeComponent();
        ...
        {
            trubleTicket = DbProvider.GetTrubleTicket(ticketId);
            userCreate = DbProvider.GetUser(trubleTicket.CreateUser);
        ...
        private void TrubleTicketForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (DialogResult == DialogResult.OK)
            {
                ...
                if (statusTrubleTicketComboBox.Text != lastStatus)
                {
                    ...
                    {
                        DbProvider.ResolveTrubleTicket(trubleTicket.Id, statusTrubleTicketComboBox.Text, resolveRichTextBox.Text, resolveUserId);
                    }
                    ...
                    {
                        DbProvider.ChangeStatusTrubleTicket(trubleTicket.Id, statusTrubleTicketComboBox.Text, resolveUserId);
                    }
                }
            }
        }
